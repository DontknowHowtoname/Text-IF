...
def read_data(root: str):
    ...
    total_dataset_nums = len(train_visible_path) + len(train_infrared_path) + len(train_visible_gt_path) + len(train_infrared_gt_path) + len(val_visible_path) + len(val_infrared_path)
...
def format_memory_usage(device):
    stats = get_memory_stats(device)
    if stats is None:
        return "mem: n/a"

    allocated_mb = stats
    return f"mem: {allocated_mb:.1f}MB"


def get_memory_stats(device):
    memory_api = _device_memory_api(device)
    if memory_api is None:
        return None

    allocated_mb = memory_api.memory_allocated(device) / 1024 / 1024
    return allocated_mb
...
def train_one_epoch(model, model_clip, optimizer, lr_scheduler, data_loader, device, epoch):
    ...
    for step, data in enumerate(data_loader):
        ...
        memory_text = format_memory_usage(device)

        data_loader.desc = "[train epoch {}] loss: {:.3f}  ssim loss: {:.3f}  max loss: {:.3f}  color loss: {:.3f}  text loss: {:.3f}  lr: {:.6f}  {}".format(
            epoch,
            accu_total_loss.item() / (step + 1),
            accu_ssim_loss.item() / (step + 1),
            accu_max_loss.item() / (step + 1),
            accu_color_loss.item() / (step + 1),
            accu_text_loss.item() / (step + 1),
            lr,
            memory_text,
        )
...
def evaluate(model, data_loader, device, epoch, lr, filefold_path):
    ...
    for step, data in enumerate(data_loader):
        ...
        data_loader.desc = "[val epoch {}] loss: {:.3f}  ssim loss: {:.3f}  max loss: {:.3f}  color loss: {:.3f}  text loss: {:.3f}  lr: {:.6f}  {}".format(
            epoch,
            accu_total_loss.item() / (step + 1),
            accu_ssim_loss.item() / (step + 1),
            accu_max_loss.item() / (step + 1),
...
def show_img(images, imagesl, B):
    ...
    for i in range(B):
        ...
        plt.imshow(np.transpose(imagesl[i], (1, 2, 0)))
        plt.axis('off')
