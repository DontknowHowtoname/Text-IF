...
from scripts.utils import (read_data, train_one_epoch_recon, evaluate_recon, create_lr_scheduler)
import datetime
import warnings
...
def main(args):
    ...
    filefold_path = "./experiments/TextIF_recon_train_{}".format(file_name)
    ...
    print(f"Training with FreeFusion reconstruction loss (upper_weight={args.upper_weight})")

    for epoch in range(start_epoch, args.epochs):
        # train
        (train_loss, train_ssim_loss, train_max_loss, train_color_loss,
         train_text_loss, train_recon_loss, lr) = train_one_epoch_recon(
            model=model,
        ...
        tb_writer.add_scalar("train_recon_loss", train_recon_loss, epoch)

        if epoch % args.val_every_epcho == 0 and epoch != 0:
            (val_loss, val_ssim_loss, val_max_loss, val_color_loss,
             val_text_loss, val_recon_loss) = evaluate_recon(
                model=model,
            ...
            tb_writer.add_scalar("val_recon_loss", val_recon_loss, epoch)
