"""
Evaluate Text_IF_Recon (FFBlock + FDBlock dual-path) model on IVT_test.
...
def main(args):
    ...
    for img_name in tqdm(image_list, desc="Evaluating"):
        ...
        try:
            ...
            with torch.no_grad():
                # Text_IF_Recon returns 5 outputs, only fused is used for evaluation
                fused, recon_ir, recon_vis, recon_dec_ir, recon_dec_vis = model(vis_tensor, ir_tensor, text)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate Text_IF_Recon (FFBlock+FDBlock) on IVT_test")
    parser.add_argument("--data_path", type=str, default="data/IVT_test", help="Path containing ir/ and vis/")
    parser.add_argument("--weights_path", type=str,
                        default="experiments/TextIF_full_recon_20260430-085722/weights/checkpoint.pth",
                        help="Text_IF_Recon model weight path")
    parser.add_argument("--output_dir", type=str, default="results/textif_full_recon_eval",
