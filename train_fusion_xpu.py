...
from scripts.utils_xpu import read_data, train_one_epoch, evaluate, create_lr_scheduler, get_memory_stats
...
def main(args):
    ...
    nw = min(cpu_count, batch_size if batch_size > 1 else 0, 8)
    ...
    if args.use_dp == True and device.type == "cuda":
        model = torch.nn.DataParallel(model).to(device)
    elif args.use_dp == True:
        print("DataParallel is only enabled for CUDA. Continue with a single xpu/cpu device.")
    ...
    for epoch in range(start_epoch, args.epochs):
        train_loss, train_ssim_loss, train_max_loss, train_color_loss, train_text_loss, lr = train_one_epoch(
            model=model,
        ...
        train_memory_stats = get_memory_stats(device)
        if train_memory_stats is not None:
            tb_writer.add_scalar("train_memory_allocated_mb", train_memory_stats, epoch)

        if epoch % args.val_every_epcho == 0 and epoch != 0:
            val_loss, val_ssim_loss, val_max_loss, val_color_loss, val_text_loss = evaluate(
                model=model,
            ...
            val_memory_stats = get_memory_stats(device)
            if val_memory_stats is not None:
                tb_writer.add_scalar("val_memory_allocated_mb", val_memory_stats, epoch)
