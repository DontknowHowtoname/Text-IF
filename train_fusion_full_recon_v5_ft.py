...
def main(args):
    ...
    if args.weights != "":
        ...
        print(f"  Missing keys (random init): {[k for k in missing if not k.startswith('base.model_clip')]}")
        if unexpected:
            print(f"  Unexpected keys (ignored): {unexpected[:5]}... ({len(unexpected)} total)")

    # Freeze encoders to preserve textif-me generalization
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, default=None,
                        help='Output directory override. Default: ./experiments/TextIF_full_recon_v5_ft_<timestamp>_sp<use_spatial>')
